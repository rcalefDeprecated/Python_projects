...
def count_kmers(sequence,k,start,stop,ignore_case=True):
	"""Takes in a sequence, alphabet, and start and stop characters, and returns counts of kmers in a Counter object.
	...
	"""
	#First we initialize the Counter object that will store each kmer's
	#count of occurences.
	counts = Counter()
	...
	for kmer in simple_kmers_from_sequence(seq,k):
		counts[kmer] += 1
	return counts
...
def generate_all_kmers(k,alphabet,start,stop):
	...
	for kmer in product(alphabet,repeat=k):
		#As product() yields tuples, we must use join to get kmers as strings.
		yield ''.join(kmer)
	#Next we want to yield kmers containing k-n characters drawn from the alphabet.
	for num in xrange(1,k):
		#Generate all possible (k-n)-mers, and prepend with n start characters.
		for suffix in product(alphabet,repeat=k-num):
			kmer = (num * start) + ''.join(suffix)
			yield kmer
		#Generate all possible (k-n)-mers, and append with n stop characters.
		for prefix in product(alphabet,repeat=k-num):
			kmer= ''.join(prefix) + (num * stop)
			yield kmer
		#For kmers with both start and stop characters, we will use 'num'
		#as the number of start characters, and 'num_stops', which ranges
		#from 1 to (k - num + 1), as sequences with starts and no stops
		#were generated above, and we want at most (k-num) stop characters
		#hence (k-num+1) as the upper bound, since xrange uses a half-open
		#interval.
		for num_stops in xrange(1,k-num + 1):
			#Number of letters from alphabet is just k minus number of
			#starts and number of stops.
			mid_length = k - num - num_stops
			#Finally, generate all possible middle sections of the kmer,
...
def check_alphabet(alphabet,start,stop):
        ...
        if start in alphabet or stop in alphabet:
            print("WARNING: Alphabet contains start and/or stop characters.\n",
                file=sys.stderr)
            sys.exit(1)
        #To check if alphaber is valid, we first make a set of valid characters, and then
